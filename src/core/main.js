...
function setupHamstersEnvironment(startOptions, onSuccess) {
  hamsters.habitat.browser = typeof window === "object";
  hamsters.habitat.worker  = typeof importScripts === "function";
  hamsters.habitat.node = typeof process === "object" && typeof require === "function" && !hamsters.habitat.browser && !hamsters.habitat.worker && !hamsters.habitat.reactNative;
  hamsters.habitat.reactNative = !hamsters.habitat.node && typeof global === 'object';
...
function checkCache(fn, input, dataType) {
  let cachedResult = hamsters.cache[fn];
  if(cachedResult) {
...
function runHamster(params, fn, onSuccess, workers, aggregate, dataType, memoize, order) {
  ...
  let task = newTask(hamsters.pool.tasks.length, workers, order, dataType, fn, onSuccess);
  if(dataType) {
