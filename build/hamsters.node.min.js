	...
	var hamstersjs = function () {
	  ...
	  */
	  function hamstersjs() {
	    ...
	    this.habitatKeys = ['worker', 'sharedworker', 'legacy', 'proxies', 'reactnative', 'atomics', 'transferable', 'browser', 'shell', 'node', 'debug', 'persistence', 'importscripts', 'maxThreads', 'parentPort', 'webworker'];
	  }
	  ...
	  */
	  ...
	  _createClass(hamstersjs, [{
	    ...
	    value: function initializeLibrary(startOptions) {
	      ...
	      _logger2.default.info('Initialized using up to ' + this.habitat.maxThreads + ' threads.');
	    }
	    ...
	    */
	    ...
	    value: function scheduleTask(task, resolve, reject) {
	      ...
	      this.pool.scheduleTask(task).then(function (results) {
	        return resolve(results);
	      }).catch(function (error) {
	        return _this.logger.error(error.messsage, reject);
	      });
	    }
	    ...
	    */
	    ...
	    value: function hamstersPromise(params, functionToRun) {
	      ...
	      return new Promise(function (resolve, reject) {
	        _this2.scheduleTask(_this2.hamstersTask(params, functionToRun), resolve, reject);
	      });
	    }
	  ...
	  _createClass(habitat, [{
	    ...
	    */
	    ...
	    value: function supportsSharedWorkers() {
	      var supports = false;
	      try {
	        var workerBlob = this.generateWorkerBlob(selectHamsterWheel());
	  ...
	  _createClass(wheel, [{
	    ...
	    */
	    ...
	    value: function regularScaffold() {
	      ...
	      var putHamsterToWork = function putHamsterToWork() {
	        new Function(params.hamstersJob)();
	        returnResponse(rtn);
	      };
	
	      var returnResponse = function returnResponse(rtn) {
	        if (rtn.dataType) {
	      ...
	      var prepareTransferBuffers = function prepareTransferBuffers(rtn, buffers) {
	  ...
	  _createClass(pool, [{
	    ...
	    */
	    ...
	    value: function returnOutputAndRemoveTask(task, resolve) {
	      var output = _data2.default.getOutput(task, _habitat2.default.transferable);
	      if (task.sort) {
	    ...
	    */
	    ...
	    key: 'trainHamster',
	    value: function trainHamster(pool, threadId, task, hamster, resolve, reject) {
	      ...
	      var onThreadResponse = function onThreadResponse(message) {
	        ...
	        task.output[threadId] = _habitat2.default.reactNative ? JSON.parse(message.data.data) : message.data.data;
	        ...
	        if (!_habitat2.default.persistence && !_habitat2.default.webWorker) {
	          hamster.terminate(); //Kill the thread only if no items waiting to run (20-22% performance improvement observed during testing, repurposing threads vs recreating them)
	        }
	      };
	      // Handle error response from a thread
	      var onThreadError = function onThreadError(error) {
	      ...
	      } else if (_habitat2.default.node) {
	        console.log("ITS NODE!! ", hamster.parentPort, _habitat2.default.parentPort);
	        hamster.parentPort.onmessage = onThreadResponse;
	        _habitat2.default.parentPort.onmessageerror = onThreadError;
	        _habitat2.default.onerror = onThreadError;
	      } else {
	  ...
	  */
	  ...
	  _createClass(data, [{
	    ...
	    value: function messageWorker(hamster, hamsterFood) {
	      ...
	      var preparedTransfer = this.prepareTransferBuffers(hamsterFood);
	    ...
	    */
	    ...
	    value: function prepareTransferBuffers(hamsterFood, transferable) {
	      var buffers = [];
	      var key = void 0,
	          newBuffer = void 0;
	      if (_habitat2.default.transferable) {
	        var _iteratorNormalCompletion = true;
	        var _didIteratorError = false;
	        var _iteratorError = undefined;
	    ...
	    */
	    ...
	    key: 'processDataType',
	    value: function processDataType(dataType, buffer, transferable) {
	      if (transferable) {
	    ...
	    */
	    ...
	    key: 'prepareOutput',
	    value: function prepareOutput(task, transferable) {
	      if (task.threads === 1) {
	    ...
	    */
	    ...
	    value: function aggregateThreadOutputs(input, dataType, transferable) {
	      ...
	      var output = this.processDataType(dataType, bufferLength, transferable);
	      var offset = 0;
	      for (i = 0; i < len; i += 1) {
