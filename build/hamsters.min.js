	...
	*/
	...
	var hamstersjs = function () {
	  ...
	  _createClass(hamstersjs, [{
	    ...
	    value: function prepareAndSaveTaskOptions(params, functionToRun, onSuccess, numberOfWorkers, aggregateThreadOutputs, dataType, memoize, sortOrder) {
	      var arrayType = 'na';
	      if (params.dataType) {
	    ...
	    key: 'runHamsters',
	    value: function runHamsters(params, functionToRun, onSuccess, numberOfWorkers, aggregateThreadOutputs, dataType, memoize, sortOrder) {
	      ...
	      var task = this.prepareAndSaveTaskOptions(params, functionToRun, onSuccess, workerCount, aggregateThreadOutputs, dataType, memoize, sortOrder);
	      this.hamstersWork(task).then(function (results) {
	        onSuccess(results);
	      }).catch(function (error) {
	        var errorMessage = 'Hamsters.js Error @ ' + error.timeStamp + ' : ' + error.message;
	    ...
	    value: function hamstersWork(task) {
	      ...
	      return new Promise(function (resolve, reject) {
	        ...
	        while (i < task.threads) {
	          _this2.wheel(i, task, resolve, reject);
	          i += 1;
	        }
	      });
	    }
	    ...
	    key: 'hamsterWheel',
	    value: function hamsterWheel(threadId, task, resolve, reject) {
	...
	*/
	...
	var habitat = function () {
	  function habitat() {
	    ...
	    this.browser = this.isBrowser(), this.worker = this.isWorker(), this.node = this.isNode(), this.reactNative = this.isReactNative(), this.shell = this.isShell(), this.transferrable = this.supportsTransferrableObjects(), this.legacy = this.isLegacyEnvironment(), this.atomics = this.supportsAtomicOperations(), this.proxies = this.supportsProxies();
	  }
