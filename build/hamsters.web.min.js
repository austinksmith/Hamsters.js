	  ...
	  */
	  ...
	  _createClass(Pool, [{
	    ...
	    */
	    ...
	    value: function runDistributedTask(incomingMessage) {
	  ...
	  _createClass(Distribute, [{
	    ...
	    value: function displayClientList() {
	      ...
	      this.connectionTargets.forEach(function (clientId) {
	        ...
	        _this2.clientSelect.appendChild(option);
	      });
	    }
	  }, {
	    key: 'loadClientList',
	    value: function loadClientList() {
	      ...
	      fetch('/clients?currentId=' + this.clientId).then(function (response) {
	        ...
	        _this3.connectionTargets = data.map(function (client) {
	          return client.id;
	        });
	        _this3.createConnection(); // Automatically create connections after fetching client list
	    ...
	    value: function createConnection() {
	      ...
	      this.connectionTargets.forEach(function (targetClient) {
	        var localConnection = new RTCPeerConnection(servers, _this4.pcConstraint);
	        var sendChannel = localConnection.createDataChannel('hamstersjs', _this4.dataConstraint);
	        ...
	        sendChannel.onopen = function () {
	          _this4.onSendChannelStateChange();
	          _this4.sendData({ targetClient: targetClient, data: 'Initial connection established' }); // Send initial data
	        };
	        sendChannel.onclose = _this4.onSendChannelStateChange.bind(_this4);
	
	        // Store connections and channels
	        _this4.remoteConnections[targetClient] = localConnection;
	        _this4.sendChannels[targetClient] = sendChannel;
	
	        localConnection.createOffer().then(function (desc) {
	          localConnection.setLocalDescription(desc);
	          _this4.ws.send(JSON.stringify({ type: 'offer', target: targetClient, offer: desc }));
	        }).catch(_this4.onCreateSessionDescriptionError);
	      });
	    }
	  }, {
	    key: 'handleOffer',
	    value: function handleOffer(data) {
	      ...
	      remoteConnection.ondatachannel = function (event) {
	        _this5.receiveChannelCallback(event, targetClient); // Pass targetClient to associate with the channel
	      };
	      ...
	      remoteConnection.setRemoteDescription(new RTCSessionDescription(data.offer)).then(function () {
	        ...
	        _this5.ws.send(JSON.stringify({ type: 'answer', target: targetClient, answer: desc }));
	    ...
	    value: function onReceiveMessageCallback(event) {
	      ...
	      var targetClient = this.findTargetClientByReceiveChannel(event.target);
	      if (targetClient) {
