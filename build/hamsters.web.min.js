...
/***/ (function(module, exports, __webpack_require__) {
	...
	***********************************************************************************/
	...
	var _memoize = __webpack_require__(8);
	
	var _memoize2 = _interopRequireDefault(_memoize);
	
	var _distribute = __webpack_require__(11);
	  ...
	  _createClass(Habitat, [{
	    ...
	    */
	    ...
	    value: function getHabitatKeys() {
	      return ['worker', 'sharedworker', 'legacy', 'proxies', 'reactnative', 'atomics', 'transferable', 'browser', 'shell', 'node', 'debug', 'persistence', 'importscripts', 'maxthreads', 'parentport', 'webworker', 'relay'];
	    }
	  }]);
	  ...
	  _createClass(Pool, [{
	    ...
	    */
	    ...
	    value: function prepareMeal(index, subTaskId, task) {
	      ...
	      var hamsterFood = {
	        array: task.input.array && task.input.array.length !== 0 ? this.hamsters.data.getSubArrayFromIndex(index, task.input.array) : [],
	        index: index
	      };
	    ...
	    */
	    ...
	    value: function runDistributedTask(incomingMessage, targetClient) {
	      ...
	      var handleResponse = this.hamsters.distribute.returnDistributedOutput;
	    ...
	    */
	    ...
	    key: 'processDistributedReturn',
	    value: function processDistributedReturn(data) {
	      debugger;
	    }
	    ...
	    */
	    ...
	    value: function trainHamster(index, task, threadId, hamster, resolve, reject) {
	      ...
	      var onThreadResponse = function onThreadResponse(message) {
	        ...
	        if (!_this.hamsters.habitat.persistence) {
	          hamster.terminate();
	        }
	        if (_this.hamsters.pool.pending.length !== 0) {
	          var queueHamster = _this.hamsters.pool.fetchHamster(_this.hamsters.pool.running.length);
	          _this.hamsters.pool.processQueuedItem(queueHamster, _this.hamsters.pool.pending.shift());
	        }
	      };
	  ...
	  */
	  ...
	  _createClass(Data, [{
	    ...
	    */
	    ...
	    value: function getSubArrayUsingIndex(index, inputArray) {
	  ...
	  _createClass(Wheel, [{
	    ...
	    value: function regularScaffold() {
	      ...
	      function getTransferableObjects(obj) {
	        var transferableObjects = new Set();
	        var typedArrayTypes = ['Int32Array', 'Uint8Array', 'Uint8ClampedArray', 'Int16Array', 'Uint16Array', 'Uint32Array', 'Float32Array', 'Float64Array'];
	        var otherTransferables = ['ArrayBuffer', 'MessagePort', 'ImageBitmap', 'OffscreenCanvas'];
	
	        var globalContext = typeof self !== 'undefined' ? self : window;
	
	        var allTypes = [].concat(typedArrayTypes, otherTransferables);
	        ...
	        for (var prop in obj) {
	          if (obj.hasOwnProperty(prop)) {
	            ...
	            try {
	              for (var _iterator = allTypes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	                var type = _step.value;
	...
	***********************************************************************************/
	...
	var Task = function () {
	  ...
	  _createClass(Task, [{
	    ...
	    key: 'setupThreadMetrics',
	    value: function setupThreadMetrics() {
	      var i = 0;
	      var threadMetrics = [];
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _NodeStore = __webpack_require__(9);
	
	var _NodeStore2 = _interopRequireDefault(_NodeStore);
	
	var _WebStore = __webpack_require__(10);
	  ...
	  _createClass(Distribute, [{
	    ...
	    value: function initWebSocket() {
	      ...
	      this.ws.onopen = function () {
	        console.info('Hamsters.js ' + _this.hamsters.version + ' connection established');
	        var registerMessage = {
	          type: 'register',
	          logicalCores: _this.hamsters.maxThreads
	        };
	        _this.ws.send(JSON.stringify(registerMessage));
	      };
	      ...
	      this.ws.onmessage = function (event) {
	        ...
	        switch (message.type) {
	          case 'register':
	            _this.handleClientReconnect(message);
	            break;
	          ...
	          case 'candidate':
	            ...
	            break;
	          case 'task-response':
	            _this.handleTaskResponse(message);
	            break;
	          default:
	            if (_this.hamsters.habitat.debug) {
	              console.info('Hamsters.js ' + _this.hamsters.version + ' unknown message type: ' + message.type);
	            }
	        }
	      };
	      ...
	      this.ws.onerror = function (error) {
	        if (_this.hamsters.habitat.debug) {
	          console.error('Hamsters.js ' + _this.hamsters.version + ' connection error: ' + error);
	        }
	      };
	
	      this.ws.onclose = function () {
	        if (_this.hamsters.habitat.debug) {
	          console.info('Hamsters.js ' + _this.hamsters.version + ' connection closed.');
	        }
	      };
	    }
	  }, {
	    ...
	    value: function updateClientList(clients) {
	      ...
	      clients.forEach(function (client) {
	        if (client.id !== _this2.clientId && !_this2.remoteConnections[client.id]) {
	          _this2.createConnection(client.id);
	        }
	      });
	    }
	  }, {
	    key: 'handleClientReconnect',
	    value: function handleClientReconnect(message) {
	      var newClientId = message.id;
	    ...
	    value: function loadClientList() {
	      ...
	      fetch('/clients?currentId=' + this.clientId).then(function (response) {
	        ...
	        _this3.updateClientList(data);
	      }).catch(function (error) {
	        if (_this3.hamsters.habitat.debug) {
	          console.error('Hamsters.js ' + _this3.hamsters.version + ' Error fetching client list: ' + error);
	        }
	      });
	    }
	  }, {
	    ...
	    value: function createConnections() {
	      ...
	      Object.keys(this.remoteConnections).forEach(function (targetClient) {
	        _this4.createConnection(targetClient);
	      });
	    }
	  }, {
	    key: 'createConnection',
	    value: function createConnection(targetClient) {
	      var _this5 = this;
	      ...
	      var localConnection = new RTCPeerConnection(servers, this.pcConstraint);
	      var sendChannel = localConnection.createDataChannel('hamstersjs', this.dataConstraint);
	
	      localConnection.onicecandidate = function (e) {
	        if (e.candidate) {
	          _this5.ws.send(JSON.stringify({ type: 'candidate', target: targetClient, candidate: e.candidate }));
	        }
	      };
	
	      sendChannel.onopen = function () {
	        _this5.onSendChannelStateChange(targetClient);
	      };
	      ...
	      sendChannel.onmessage = function (event) {
	        ...
	        switch (message.type) {
	          case 'ping':
	            _this5.handlePing(targetClient, message.startTime);
	            break;
	          case 'pong':
	            _this5.handlePong(targetClient, message.startTime);
	            break;
	          default:
	            _this5.onReceiveMessageCallback(targetClient, event.data);
	        }
	      };
	
	      localConnection.ondatachannel = function (event) {
	        _this5.receiveChannelCallback(event, targetClient);
	      };
	      ...
	      localConnection.createOffer().then(function (desc) {
	        ...
	        _this5.ws.send(JSON.stringify({ type: 'offer', target: targetClient, offer: desc }));
	    ...
	    value: function handleOffer(data) {
	      ...
	      if (!this.remoteConnections[targetClient]) {
	        ...
	        var remoteConnection = new RTCPeerConnection(servers, this.pcConstraint);
	
	        remoteConnection.onicecandidate = function (e) {
	          if (e.candidate) {
	            _this6.ws.send(JSON.stringify({ type: 'candidate', target: targetClient, candidate: e.candidate }));
	          }
	        };
	
	        remoteConnection.ondatachannel = function (event) {
	          _this6.receiveChannelCallback(event, targetClient);
	        };
	
	        var sendChannel = remoteConnection.createDataChannel('hamstersjs', this.dataConstraint);
	
	        sendChannel.onopen = function () {
	          _this6.onSendChannelStateChange(targetClient);
	        };
	        ...
	        sendChannel.onmessage = function (event) {
	          ...
	          switch (message.type) {
	            case 'ping':
	              _this6.handlePing(targetClient, message.startTime);
	              break;
	            case 'pong':
	              _this6.handlePong(targetClient, message.startTime);
	              break;
	            default:
	              _this6.onReceiveMessageCallback(targetClient, event.data);
	          }
	        };
	        ...
	        remoteConnection.setRemoteDescription(new RTCSessionDescription(data.offer)).then(function () {
	          ...
	          _this6.ws.send(JSON.stringify({ type: 'answer', target: targetClient, answer: desc }));
	        }).catch(this.onCreateSessionDescriptionError);
	    ...
	    key: 'measureLatency',
	    value: function measureLatency(targetClient) {
	      var startTime = performance.now();
	      this.sendPing(targetClient, startTime);
	    }
	  }, {
	    key: 'sendPing',
	    value: function sendPing(targetClient, startTime) {
	      var sendChannel = this.sendChannels[targetClient];
	      if (sendChannel && sendChannel.readyState === 'open') {
	        sendChannel.send(JSON.stringify({ type: 'ping', startTime: startTime }));
	      }
	    }
	  }, {
	    key: 'handlePing',
	    value: function handlePing(targetClient, startTime) {
	      console.log("Remote Connections ", this.remoteConnections);
	      var sendChannel = this.receiveChannels[targetClient];
	      if (sendChannel && sendChannel.readyState === 'open') {
	        sendChannel.send(JSON.stringify({ type: 'pong', startTime: startTime }));
	        if (this.hamsters.habitat.debug) {
	    ...
	    key: 'handlePong',
	    value: function handlePong(targetClient, startTime) {
	    ...
	    value: function fetchDistributedClient() {
	      ...
	      sendChannelKeys.forEach(function (clientId) {
	        var channel = _this7.sendChannels[clientId];
	        if (channel.readyState === 'open' && _this7.latencies[clientId] < minLatency) {
	          minLatency = _this7.latencies[clientId];
	    ...
	    value: function distributeTask(task, hamsterFood, resolve, reject) {
	      ...
	      var messageId = this.generateUniqueId();
	      ...
	      this.pendingPromises[messageId] = { resolve: resolve, reject: reject };
	
	      this.sendData({ targetClient: targetClient, data: subTask });
	    }
	  }, {
	    key: 'handleTaskResponse',
	    value: function handleTaskResponse(incomingMessage) {
	      var messageId = incomingMessage.messageId,
	          output = incomingMessage.output,
	          error = incomingMessage.error;
	
	      var pendingPromise = this.pendingPromises[messageId];
	    ...
	    key: 'sendData',
	    value: function sendData(_ref) {
	      var targetClient = _ref.targetClient,
	          data = _ref.data;
	    ...
	    value: function sendDataResponse(data) {
	    ...
	    value: function receiveChannelCallback(event, targetClient) {
	      ...
	      receiveChannel.onmessage = function (event) {
	        ...
	        switch (message.type) {
	          case 'ping':
	            _this8.handlePing(targetClient, message.startTime);
	            break;
	          case 'pong':
	            _this8.handlePong(targetClient, message.startTime);
	            break;
	          default:
	            _this8.onReceiveMessageCallback(targetClient, event.data);
	        }
	      };
	
	      receiveChannel.onopen = function () {
	        _this8.onReceiveChannelStateChange(targetClient);
	      };
	    ...
	    value: function onSendChannelStateChange(targetClient) {
	      var sendChannel = this.sendChannels[targetClient];
	    ...
	    value: function onReceiveChannelStateChange(targetClient) {
	      var receiveChannel = this.receiveChannels[targetClient];
