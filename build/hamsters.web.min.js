	...
	var Pool = function () {
	  ...
	  */
	  ...
	  _createClass(Pool, [{
	    ...
	    value: function addWorkToPending(index, subTaskId, task, resolve, reject) {
	    ...
	    */
	    ...
	    key: 'runTask',
	    value: function runTask(hamster, index, subTaskId, task, resolve, reject) {
	  ...
	  _createClass(Distribute, [{
	    ...
	    value: function initWebSocket() {
	      ...
	      this.ws.onmessage = function (event) {
	        ...
	        switch (message.type) {
	          case 'register':
	            _this2.clientId = message.id;
	            _this2.loadClientList();
	            break;
	          case 'update-client-list':
	            _this2.updateClientList(message.clients);
	            break;
	          case 'offer':
	          ...
	          case 'answer':
	            ...
	            break;
	          case 'candidate':
	            _this2.handleCandidate(message);
	            break;
	          default:
	    ...
	    key: 'displayClientList',
	    value: function displayClientList() {
	      var _this3 = this;
	
	      // Ensure that this method clears the list before updating to avoid duplicates
	      this.clientSelect.innerHTML = '';
	      this.connectionTargets.forEach(function (clientId) {
	        var option = document.createElement('option');
	        option.value = clientId;
	        option.text = clientId;
	        _this3.clientSelect.appendChild(option);
	      });
	    }
	  }, {
	    key: 'loadClientList',
	    value: function loadClientList() {
	      ...
	      fetch('/clients?currentId=' + this.clientId).then(function (response) {
	        ...
	        _this4.connectionTargets = data.map(function (client) {
	          return client.id;
	        });
	        _this4.createConnection(); // Automatically create connections after fetching client list
	    ...
	    value: function createConnection() {
	      ...
	      this.connectionTargets.forEach(function (targetClient) {
	        var localConnection = new RTCPeerConnection(servers, _this5.pcConstraint);
	        var sendChannel = localConnection.createDataChannel('hamstersjs', _this5.dataConstraint);
	        ...
	        sendChannel.onopen = function () {
	          _this5.onSendChannelStateChange();
	          _this5.sendData({ targetClient: targetClient, data: 'Initial connection established' }); // Send initial data
	        };
	        sendChannel.onclose = _this5.onSendChannelStateChange.bind(_this5);
	
	        // Store connections and channels
	        _this5.remoteConnections[targetClient] = localConnection;
	        _this5.sendChannels[targetClient] = sendChannel;
	        ...
	        }).catch(_this5.onCreateSessionDescriptionError);
	      });
	    }
	  }, {
	    key: 'handleOffer',
	    ...
	    value: function sendData(data) {
	      ...
	      if (sendChannel.readyState === 'open') {
	        ...
	        sendChannel.onopen = function () {
	          sendChannel.send(JSON.stringify(payload));
	          _this7.trace('Sent Data to ' + targetClient + ': ' + payload);
	        };
	      }
	    }
	  }, {
