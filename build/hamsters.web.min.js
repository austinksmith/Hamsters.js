	  ...
	  _createClass(Distribute, [{
	      ...
	      this.connectionTargets.forEach(function (targetClient) {
	        if (!_this3.remoteConnections[targetClient]) {
	          ...
	          sendChannel.onopen = function () {
	            _this3.onSendChannelStateChange(targetClient);
	          };
	          ...
	          sendChannel.onmessage = function (event) {
	            ...
	            switch (message.type) {
	              case 'ping':
	                _this3.handlePing(targetClient, message.startTime);
	                break;
	              case 'pong':
	                _this3.handlePong(targetClient, message.startTime);
	                break;
	              default:
	                _this3.onReceiveMessageCallback(targetClient, event.data); // Use `self` to refer to `Distribute` instance
	            }
	          };
	    ...
	    value: function fetchDistributedClient() {
	      ...
	      sendChannelKeys.forEach(function (clientId) {
	        if (_this5.latencies[clientId] < minLatency) {
	          minLatency = _this5.latencies[clientId];
	    ...
	    value: function handleTaskResponse(data) {
	      var messageId = data.messageId,
	          output = data.output,
	          error = data.error;
	      var _pendingPromises$mess = this.pendingPromises[messageId],
	          resolve = _pendingPromises$mess.resolve,
	          reject = _pendingPromises$mess.reject;
	    ...
	    value: function receiveChannelCallback(event, targetClient) {
	      ...
	      receiveChannel.onopen = function () {
	        _this7.onReceiveChannelStateChange(targetClient);
	      };
